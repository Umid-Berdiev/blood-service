...
export interface SterilityControlItemInterface {
  date: string
...
useHead({
  title: `${t('Components_for_production_sterility_control')} - ${mainStore.app.name}`,
})

const apiData: ApiDataInterface<SterilityControlItemInterface> = reactive({
  data: [],
  pagination: {
    total: 10,
    count: 10,
...
function openBuckControlSterilityFormModal(patient: SterilityControlItemInterface) {
      ...
      <div class="column">
        <VBreadcrumb
          ...
          :items="[
            ...
            {
              label: $t('Components_for_production_sterility_control'),
            },
          ]"
        />
      </div>
